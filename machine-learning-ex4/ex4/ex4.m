...
% Select and print labels for these 100 randomly selected points
sel_labels = y(sel);
sel_lables = reshape(sel_labels, 10, 10)'

% disply randomly selected units
displayData(X(sel, :));
...
for i = 1:length(total_iters),
    ...
    pred = predict(Theta1, Theta2, X);
    accuracy_by_lambda(i) = mean(double(pred == y)) * 100;
end;

plot(lambda_vals, accuracy_by_lambda, 'rx', 'MarkerSize', 10);
ylabel('Accuracy achieved with specified lambda');
xlabel('lambda');
